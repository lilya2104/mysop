...
public class GreenhouseDataFetcher {
    ...
    // Получение теплицы по идентификатору. Соответствует полю Query.greenhouse(id: ID!) в схеме.
    @DgsQuery
    public GreenhouseResponse greenhouse(@InputArgument String id) {
        return greenhouseService.findById(Long.parseLong(id));
    }
    // Список авторов с пагинацией. Соответствует полю Query.greenhouses(page, size) в схеме.
    @DgsQuery
    public GreenhouseConnectionGql greenhouses(
        ...
        PagedResponse<GreenhouseResponse> paged = greenhouseService
                .findAll(searchPlantName, pageNum, pageSize);
    ...
    // Создание теплицы. Соответствует полю Mutation.createGreenhouse(input) в схеме.
    @DgsMutation
    ...
    // Обновление теплицы. Соответствует полю Mutation.updateGreenhouse(id, input) в схеме.
    @DgsMutation
    public GreenhouseResponse updateGreenhouse(@InputArgument String id, @InputArgument UpdateGreenhouseInputGql input) {
        ...
        return greenhouseService.updateGreenhouse(Long.parseLong(id), request);
    }
    // Полив теплицы. Соответствует полю Mutation.wateringGreenhouse(id) в схеме.
    @DgsMutation
    ...
    @DgsMutation
    public GreenhouseResponse changeWateredStatus(@InputArgument String id,
                                                  @InputArgument WateredStatus newStatus) {
        return greenhouseService.changeWateredStatus(Long.parseLong(id), newStatus);
    }
    // Удаление теплицы. Соответствует полю Mutation.deleteGreenhouse(id) в схеме.
    @DgsMutation
