...
import edu.rutmiit.demo.grpcanalytics.service.GreenhouseAnalyticsServiceImpl;
    ...
    public void start() {
        try {
            server = ServerBuilder.forPort(grpcPort)
                    .addService(new GreenhouseAnalyticsServiceImpl())
                    .build()
                    .start();

            running = true;
            log.info("gRPC-сервер запущен на порту {}", grpcPort);
            log.info("Сервис: GreenhouseAnalytics.AnalyzeGreenhouse()");
